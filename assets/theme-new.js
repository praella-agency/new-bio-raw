          ...
          $.each(cart.items, function(index, product) {
            ...
            if (product.properties !== null) {
              $.each(product.properties, function(key, value) {
                if (key.charAt(0) === '_' || !value || key.indexOf('Meal') > -1 || key == 'shipping_interval_frequency' || key == 'shipping_interval_unit_type' || key == 'charge_interval_frequency' || key == 'subscription_id') {
            ...
            let titleProduct = product.product_title;
            
            if (product.variant_title != 'subscription') {
              titleProduct = product.product_title.split('(weekly')[0];
            }
            
            if (product.properties['_random_number'] != undefined) {
              if(product.properties['_product_type'] == 'Main Product'){
//                 $(document).ready(function() {
//                   $(document).find(`[data-prdTitle="${product.product_title}"]`).addClass('showAddon');
//                   })
              url = '/pages/our-meals'; 
              } else if (product.properties['_product_type'] == 'Sub Product'){
                 url = '/pages/our-meals';
//                 $('html, body').animate({
      ...
      jQuery.getJSON('/search/suggest.json', {
        ...
        "resources": {
          ...
          "options": {
            ...
            "fields": "title,_product_type,variants.title,vendor"
          }
        }
...
//         }
  ...
  theme.HeaderSection = (function() {
    ...
    Header.prototype = $.extend({}, Header.prototype, {
      ...
      // console.log(theme.settings.cartType)
//         if (theme.settings.cartType === 'drawer') {
...
//         }
  ...
  $('.cart_redirect_checkout').on('click',function(event){
    ...
    axios.get('/cart.js').then((response) => {
      ...
      console.log(theme.settings.customersID);
      ...
      if (isRechargeProduct) {
        ...
        // var 
        // let form_action = $('[data-cart-form]').attr('action');
        ...
        var customer_param = '';
        var checkout_url = `https://checkout.rechargeapps.com/r/checkout?myshopify_domain=${theme.settings.myshopify_domain}&cart_token=${getCartCookie}&${ga_linker}&${customer_param}`;   
  ...
  $(document).ready(function() {
    ...
    $('<style>#submitButton {font-weight: 700 !important; }</style>').wrap('<head></head>')
